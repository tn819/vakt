...
export function registerProxy(program: Command): void {
  program
    ...
    .action(async (serverName: string) => {
      ...
      // Server stdout → agent stdout (passthrough, no modification)
      void (async () => { for await (const chunk of proc.stdout) process.stdout.write(chunk); })();

      // Server stderr → our stderr
      void (async () => { for await (const chunk of proc.stderr) process.stderr.write(chunk); })();

      void proc.exited.then(code => process.exit(code ?? 0));
    });
}
